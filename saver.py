...
def save_recognition(name, frame):
    if not settings.SAVE_DETECTION_STATUS:
        return
    current_time = time.time()

    if not settings.last_saved_time.__contains__(name) or current_time - settings.last_saved_time[name] >= settings.SAVE_DELAY_MS / 1000:
        timestamp = time.strftime("%Y%m%d_%H%M%S")  # Текущее время как часть имени файла
        filename = f"{name}_{timestamp}.jpg"

        filepath = os.path.join(settings.TEMP_PATH, filename)
        save_result = save_frame_on_disk(frame, filepath)

        if save_result:
            #      print(f"[INFO] Сохранено изображение {filename}")
            settings.last_saved_time[name] = current_time  # Обновляем время последнего сохранения
...
def save_data_on_disk():
    with open(settings.KNOWN_FACES_FILE, "wb") as f:
        pickle.dump((settings.known_face_encodings, settings.known_face_names, settings.known_face_images), f)
    with open(settings.SAVED_FORM_ANSWERS_FILE, "w") as f:
        json.dump(settings.saved_form_answers, f)
    with open(settings.BLOCKED_GOOGLE_FILES_FILE, "w") as f:
        json.dump(settings.blocked_google_files, f)


def forget_face(name, j):
    try:
        i = settings.known_face_names.index(name)
        filepath = settings.known_face_images[i].pop(j)
        settings.known_face_encodings[i].pop(j)
        os.remove(filepath)
...
def forget_all_faces(name):
    try:
        i = settings.known_face_names.index(name)
        while len(settings.known_face_images[i]) > 0:
            forget_face(name, 0)
    except ValueError:
...
def clean():
    rem = []
    for idx, name in enumerate(settings.known_face_names):
        if len(settings.known_face_encodings[idx]) == 0:
            rem.append((idx, name))
    rem.reverse()
    for idx, name in rem:
        try:
            os.rmdir(os.path.join(settings.SAVED_FRAMES_FOLDER, name))
        except FileNotFoundError:
            print(f'Folder for {name} don\'t exist')
        except OSError:
            print(f'Not all files for {name} deleted')
        settings.known_face_names.pop(idx)
        settings.known_face_encodings.pop(idx)
        settings.known_face_images.pop(idx)
    save_data_on_disk()
...
def save_from_encoding(face_encoding, name, frame, filename, additional_info=None):
    ...
    if name not in settings.known_face_names:
        settings.known_face_names.append(name)
        settings.known_face_encodings.append([])
        settings.known_face_images.append([])

    index = settings.known_face_names.index(name)
    settings.known_face_encodings[index].append(face_encoding)
    settings.known_face_images[index].append(filepath)


def save_from_frame(frame, name: str, filename, additional_info=None):
    rgb_frame = get_rgb_frame(frame)
    image = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb_frame)
    landmarkers = settings.landmarker_image.detect(image)
...
def save_from_file_and_move(name, path, filename, additional_info=None):
    filepath = os.path.join(settings.NEW_FRAMES_FOLDER, path)
    movefilepath = os.path.join(settings.OLD_FRAMES_FOLDER, path)
    try:
        frame = read_frame_file(filepath)
        save_from_frame(frame, name, filename, additional_info)
...
def autosave():
    for (_, dirs, _) in os.walk(settings.NEW_FRAMES_FOLDER):
        for name in dirs:
            path = str(os.path.join(settings.NEW_FRAMES_FOLDER, name))
            movepath = str(os.path.join(settings.OLD_FRAMES_FOLDER, name))
...
def main_saver():
    ...
    while True:
        ...
        if key == 'q':
            ...
            while name == settings.UNKNOWN_NAME:
            ...
            while True:
                if len(settings.known_face_names) > 0:
                    clean()
                    ...
                    if choice.lower() == 'q':
                        break
                    if choice.lower() == 'all':
                        while len(settings.known_face_names) > 0:
                            forget_all_faces(settings.known_face_names[0])
                            clean()
                        save_data_on_disk()
                        break
                    try:
                        index = int(choice) - 1
                        if 0 <= index < len(settings.known_face_names):
                            while True:
                                print("Choose frame to delete('q' - to quite, 'all' - to delete all):")
                                for idx, path in enumerate(settings.known_face_images[index]):
                                    print(f'{idx + 1} - {path}')
                                choice = input()
                                if choice == 'q':
                                    break
                                if choice == 'all':
                                    deleted_name = settings.known_face_names[index]
                                ...
                                try:
                                    j = int(choice) - 1
                                    if 0 <= j < len(settings.known_face_images[index]):
                                        name = settings.known_face_names[index]
                                        forget_face(name, j)
                                        save_data_on_disk()
            ...
            while True:
                ...
                if len(responses) > 0:
                    ...
                    for idx, response in enumerate(responses):
                        if settings.saved_form_answers.__contains__(response['responseId']):
